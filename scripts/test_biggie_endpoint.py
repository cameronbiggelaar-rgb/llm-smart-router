...
# TEST 1: biggie-llm + tools + simple coding prompt -> most capable tool model (gpt-5.5)
d = route_task(complexity_score=0.1, task_type="coding", requires_tools=True)
check("TEST1: tools + simple coding → gpt-5.5 (most capable tool model)",
...
# TEST 5: qwen cheaper/available -> still cannot win tool-required routing (must be gpt-5.5)
d = route_task(complexity_score=0.1, task_type="coding", requires_tools=True)
check("TEST5: qwen cannot win tool-required routing",
      d.selected_model == "gpt-5.5", f"got {d.selected_model}")

# TEST 6: glm cheaper/available -> still cannot win tool-required routing (gpt-5.5 wins)
d = route_task(complexity_score=0.1, task_type="coding", requires_tools=True)
