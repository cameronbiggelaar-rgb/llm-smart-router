...
- **The quality probe scores `delta["content"]` only, and the upstream stream also carries a `reasoning` channel it never reads — so a score can measure the capture path rather than the model.** `_sse_content_text` returns only `content`; `reasoning` deltas are forwarded to the client and dropped by the probe. Live production rows show the symptom independently of any harness: `quality_probe.summary_chars` sits in a **~68–647 band while the same request billed `output_tokens` from 104 to 6,412** (0.01–2.4 chars per token), so the scored text does not scale with what the model produced. **Whether `reasoning` explains the missing text is UNRESOLVED — do not assert it.** The share is prompt- and payload-dependent: a clean no-tools replay at a 20K payload gave 30.9% reasoning with 6,910 chars of content (healthy, 2.44 chars/token), replays that inherited an agent system prompt finished `tool_calls` with 0 content chars, and a live-endpoint replay returned 4,528 content chars with no `reasoning` at all. Those 0-char runs are harness contamination (finish `tool_calls`, 301–529 completion tokens vs production 4,411–6,412), not production behaviour. Replay with a production-equivalent compaction prompt before concluding. The transferable rule: check `summary_chars` against that row's `output_tokens` — if the scored text does not scale with the billed output, the score is not measuring the model.
