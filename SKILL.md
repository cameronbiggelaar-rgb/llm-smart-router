...
- **The quality probe scores `delta["content"]` only, but the upstream stream carries a SECOND text channel (`reasoning`) that it ignores — so it under-measures every reasoning-capable summariser.** The endpoint forwards upstream SSE verbatim, so the client receives both channels; `_sse_content_text` returns only `content` and `_resume_stream` accumulates only that. Measured on the live upstream path (ollama `/v1/chat/completions`, `deepseek-v4.1-flash:cloud`): **86.7% of emitted text arrived as `reasoning`** on a small payload and **100% at production scale (120K chars), where visible `content` collapsed to 0 chars**. The live consequence: `summary_chars` sits in a **~68–117 band regardless of `output_tokens`** (104 → 6,412), the signature of a fixed-size artifact rather than a model output. Before trusting any production quality number, check `summary_chars` against that row's `output_tokens` — if the scored text does not scale with what the model billed, the score measures the capture path, not the model.
