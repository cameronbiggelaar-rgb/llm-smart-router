...
- **`fact_coverage_v1` cannot rank compression summaries of large contexts — never choose a model on it alone.** It is *share of source numbers reproduced*, so a 50K-char source makes a huge denominator and any summary under ~15K chars scores near zero; it also **systematically penalises brevity** (measured: summary/source ratio 0.206 and 0.159 vs the incumbent's 0.367 → coverage 0.05 vs 0.24). The tell that this is a metric artifact rather than a model ranking: **every** model scores low on large contexts, including the incumbent.
