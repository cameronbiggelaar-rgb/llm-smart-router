...
- **The quality probe scores `delta["content"]` only, and the upstream stream carries a second `reasoning` channel it never reads — so a score can measure the capture path rather than the model.** `_sse_content_text` (endpoint ~2136) returns `content` alone; the accumulator at ~2440/2462 feeds `on_summary` from that, and `reasoning` deltas are forwarded to the client and dropped by the probe. Production evidence, free of any harness: 0 of 50 captured samples carry tool definitions, so a replay ending `finish_reason=tool_calls` means the harness passed tools (contaminated, not production). Every probed production row has `saw_content=1`, yet `summary_chars` is 57-647 for the same rows billing `output_tokens` 195-9237 (median ratio 0.063 — about 16x less text than the token count implies). A clean no-tools replay of a real 192,902-char production payload returned 0 content chars against 154 reasoning chars (259 completion tokens), so the failure mode reproduces without tools. **Whether `reasoning` fully accounts for the gap is unresolved — do not assert it.** The share is prompt- and payload-dependent. Transferable rule: check `summary_chars` against that row `output_tokens`; if the scored text does not scale with the billed output, the score is not measuring the model.
